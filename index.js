...
/******** OWNER NUMBER**********/
const ownerNumber = ["6281215525272@s.whatsapp.net","6281215524272@s.whatsapp.net"] 
/************************************/
            ...
            case 'bitly':
	          ...
	          if (!isGroup) return reply(ind.groupo())
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					if (!tex) return client.sendMessage(from, '𝐌𝐚𝐬𝐮kk𝐚𝐧 𝐓𝐞𝐤𝐬/𝐔𝐫𝐥 𝐘𝐚𝐧𝐠 𝐈𝐧𝐠𝐢𝐧 𝐃𝐢 𝐁𝐮𝐚𝐭 𝐐𝐑', text, {quoted: mek})
               ...
               if (data.error) return reply(data.error)
                ...
                case 'beritahoax':
					...
					reply('Makasih poto profil barunya😘')
					break 
					case 'brainly':
					if (!isRegistered) return reply(ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				case 'bcgc':
                   ...
                   if (!isGroup) return reply(ind.groupo())
					...
					if (!isRegistered) return reply(ind.noregis())
					client.updatePresence(from, Presence.composing) 
					teks = `\`\`\`Ini adalah list group AZKABOT :\n\n\`\`\``
					no = 0
		...
		case 'nguli':
          ...
          case 'hidetag':
			...
			case 'ohidetag':
                ...
                if (isLimit(sender)) return reply(ind.limitend(pusname))
                if (!isGroup) return reply(ind.groupo())
					if (args.length < 1) return reply('Urlnya mana om senpai ?')
					teks = body.slice(7)
					reply(ind.wait())
			...
			if (isLimit(sender)) return reply(ind.limitend(pusname))
			if (!isGroup) return reply(ind.groupo())
			if (args.length < 1) return reply('mau cari apa senpai !')
			reply(ind.wait)
			...
			if (!isGroup) return reply(ind.groupo())
                ...
                if (!isRegistered) return reply(ind.noregis())
                if (isLimit(sender)) return reply(ind.limitend(pusname))
					if (args.length < 1) return reply('Apa yang mau dicari senpai ?')
                ...
                if (isLimit(sender)) return reply(ind.limitend(pusname))
                if (!isGroup) return reply(ind.groupo())
					if (args.length < 1) return reply('Sebutkan namanya senpai !')
				...
				if (!isGroup) return reply(ind.groupo())
				try {
						if (args.length < 1) return client.sendMessage(from, 'Usernamenya mana senpai ?', text, {quoted: mek})
				...
				case 'bc': 
			   	...
			   	case 'setpp': 
                       ...
                       media = await client.downloadAndSaveMediaMessage(mek)
                        ...
                        reply('𝘀𝘂𝗸𝘀𝗲𝘀 𝗺𝗲𝗻𝗴𝗴𝗮𝗻𝘁𝗶 𝗶𝗰𝗼𝗻 𝗚𝗿𝘂𝗽')
					break						
				case 'add':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
            ...
            case 'creator':
                  ...
                  client.sendMessage(from, 'Tuh nomer pacarku , eh ownerku >_<, jangan spam atau ku block kamu',MessageType.text, { quoted: mek} )
					break    
           case 'setname':
                if (!isGroup) return reply(ind.groupo())
			    if (!isGroupAdmins) return reply(ind.admin())
